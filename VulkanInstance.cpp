...
void VulkanInstance::createLogicalDevice() {
    ...
    // Does not return a VK_RESULT, returns void but should work if the device is created
...
void VulkanInstance::createGraphicsPipeline() {
    // Shaders build to base project directory because shaders is a visual studio filter
    // not a folder
    auto vertShaderCode = readFile("vert.spv");
    auto fragShaderCode = readFile("frag.spv");
  
    // Check size of buffers against file size in bytes to see if shaders loaded correctly
    ...
    // Describe how the vertex data will be fed into the pipeline
    ...
    // Describe what kind of geometry will be drawn, and whether primitive restart is enabled
    VkPipelineInputAssemblyStateCreateInfo inputAssembly{};
    inputAssembly.sType = VK_STRUCTURE_TYPE_PIPELINE_INPUT_ASSEMBLY_STATE_CREATE_INFO;
    inputAssembly.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;
    inputAssembly.primitiveRestartEnable = VK_FALSE;

    // Viewport is 'display area size' scissor is visible part of the image
    // Setting scissor to viewport size will display the whole image across the full size of the viewport
    VkViewport viewport{};
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    ...
    // These values can be changed dynamically without recreating the pipeline at draw time if specified here
    // NOTE - if we didnt specify the dynamic states, this would become immutable and need to be recreated
    ...
    // Some graphics cards support multiple viewports and scissors, on those you access them from an array
    ...
    // This is how you perform anti aliasing
    ...
    // Colour blending for anti aliasing
    ...
    // Blende the colors based on the color Blend attachment
    ...
    // Write to the class variable pipelineLayout
    if (vkCreatePipelineLayout(logicalDevice, &pipelineLayoutInfo, nullptr, &pipelineLayout) != VK_SUCCESS) {
        throw std::runtime_error("failed to create pipeline layout!");
    }
 
    // The main graphics pipeline struct where all previous components are assembled
...
std::vector<char> VulkanInstance::readFile(const std::string& filename)
{
    ...
    std::cout <<"File size of "<< filename << ": " << fileSize << std::endl;
...
void VulkanInstance::createSyncObjects() {
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        if (vkCreateSemaphore(logicalDevice, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i])
                ...
                {
                    throw std::runtime_error("failed ot create semaphores!");
                }
    }
}
